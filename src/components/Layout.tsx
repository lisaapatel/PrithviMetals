...
const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="min-h-screen bg-[#0B1121]">
      {/* Main content wrapper with conditional blur and click handler */}
      <div 
        className={`transition-all duration-500 ${isMenuOpen ? 'blur-3xl' : 'blur-none'}`}
        onClick={() => isMenuOpen ? setIsMenuOpen(false) : undefined}
      >
        <header className="fixed top-0 left-0 right-0 z-40 bg-[#0B1121]/80 backdrop-blur-sm border-b border-gray-800">
      ...
      <div
        className={`md:hidden fixed inset-0 z-50 pointer-events-none transition-all duration-300 ease-in-out ${
          isMenuOpen ? 'visible' : 'invisible'
        }`}
      >
        {/* Menu Panel - Updated with refined blue theme */}
        <div
          className={`pointer-events-auto absolute top-0 right-0 w-[80%] max-w-sm h-full bg-gradient-to-b from-[#0B1121] via-[#0B1121] to-[#070B14] border-l border-indigo-900/30 shadow-2xl transform transition-transform duration-300 ease-in-out ${
            isMenuOpen ? 'translate-x-0' : 'translate-x-full'
          }`}
        >
          <div className="flex flex-col h-full bg-[#0B1121]/95">
            {/* Header */}
            <div className="flex items-center justify-between p-6 border-b border-indigo-900/20">
            ...
            {/* Footer */}
            <div className="p-6 border-t border-indigo-900/20">
              <a 
